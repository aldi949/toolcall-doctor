"""Run one DDMin arm. Writes CANDIDATE_FROZEN.json. Does NOT open holdout."""
...
def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in {"baseline", "robust"}:
        print("usage: run_arm.py baseline|robust")
        return 2
    arm = sys.argv[1]
    ...
    out = EXP / arm
    session = Session(out, arm, n, facts, exec_spec)
    print("phase search", arm, "n", n, "atoms", len(atoms), flush=True)
    t0 = datetime.now(timezone.utc)
    mini = ddmin(original, atoms, session)
    t1 = datetime.now(timezone.utc)
    print("ddmin", arm, mini.get("status"), "http", mini.get("http_calls"), flush=True)
    ...
    print("CANDIDATE FROZEN", arm, flush=True)
