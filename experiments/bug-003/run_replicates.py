"""Run control then broken probes N times."""
...
def run_one(request_name: str, stem: str) -> dict:
    ...
    (ROOT / "raw" / f"{stem}.stderr.txt").write_text(p.stderr or "", encoding="utf-8")
    print(p.stdout, end="")
    if p.returncode != 0:
...
def main() -> int:
    ...
    (ROOT / "raw" / "replication_index.json").write_text(json.dumps(results, indent=2) + "\n", encoding="utf-8")
