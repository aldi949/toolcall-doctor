"""1-minimality (same 10/10) and standalone (execute.post, fresh process)."""
...
def verify_1min() -> dict:
    ...
    result = json.loads((EXP / "minimization" / "search" / "ddmin_result.json").read_text(encoding="utf-8"))
    remaining = list(result.get("current_ids") or [])
    atoms = extract_atoms(original)
    by = {a.atom_id: a for a in atoms}
    n = int(POLICY["search_n"])
    session = Session(EXP / "minimization", "minimization", n, facts, exec_spec)
    probes = []
    still = []
    ...
    dump(EXP / "verification" / "ONE_MIN.json", out)
    return out


def standalone() -> dict:
    frozen = json.loads((EXP / "minimization" / "CANDIDATE_FROZEN.json").read_text(encoding="utf-8"))
    payload = frozen["payload"]
    n = int(POLICY["standalone_n"])
    need = int(POLICY["standalone_pass_k"])
    script = EXP / "standalone" / "run_one.py"
    ...
    for i in range(1, n + 1):
        ...
        if ev:
            k += 1
        rows.append({"i": i, "event": ev, "tool_name": ora.get("tool_name"), "arguments": ora.get("arguments"), "returncode": p.returncode})
        print("standalone", i, ev, ora.get("tool_name"), flush=True)
    out = {"k": k, "n": n, "pass": k >= need, "need": need, "rows": rows, "utc": utc_now()}
    dump(EXP / "standalone" / "STANDALONE.json", out)
    return out


def verify_pool() -> dict:
    frozen_path = EXP / "minimization" / "CANDIDATE_FROZEN.json"
    if not frozen_path.is_file():
        raise RuntimeError("verification before candidate freeze")
    out_path = EXP / "verification" / "VERIFICATION.json"
    if out_path.is_file():
    ...
    exec_spec = json.loads((EXP / "engine" / "EXEC_SPEC.json").read_text(encoding="utf-8"))
    n = int(POLICY["search_n"])
    res = run_pool(payload, EXP / "verification" / "raw", n, facts, exec_spec)
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: run_verify.py 1min|standalone|verify")
        return 2
    cmd = sys.argv[1]
    if cmd == "1min":
        one = verify_1min()
        ...
        return 0
    if cmd == "standalone":
        print(json.dumps(standalone()))
        return 0
    if cmd == "verify":
        res = verify_pool()
        print("VERIFY", f"{res['k_events']}/{res['n']}", flush=True)
        return 0
    return 2
