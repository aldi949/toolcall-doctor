"""HTTP POST + fingerprint. Used by search, holdout, standalone. No urllib variant."""
...
def fingerprint_static() -> dict:
    ...
    try:
        with httpx.Client(timeout=5.0) as c:
            ver = c.get("http://127.0.0.1:11434/api/version").json().get("version")
...
def post(payload: dict, artifact_dir: Path, timeout: float = 60.0) -> dict:
