...
This file is hashed into `FREEZE_MANIFEST.json`. It must not change after that hash.
...
`oracle == FAIL` and `failure_identity == HTTP_400_UNMARSHAL_TYPE_ARRAY_INTO_STRING`.

Every test is one new candidate ID, one HTTP POST, one ledger line. Candidate
...
`effective(C \ {a})` once. If any such test still preserves the target
identity, the result is **not** 1-minimal in this space.
