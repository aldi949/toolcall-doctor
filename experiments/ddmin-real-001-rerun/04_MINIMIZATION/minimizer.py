...
def run_test(
    ...
    *,
    ...
    exe = post(payload, raw)
    ora = evaluate(exe["status"], exe["text"])
    (raw / "oracle.json").write_text(json.dumps(ora, indent=2) + "\n", encoding="utf-8")
    ...
    rec = {
        ...
        "payload_sha256": exe["request_sha256"],
        "compact_bytes": exe["compact_bytes"],
        "timestamp_start": exe["started_utc"],
        "timestamp_end": exe["ended_utc"],
        ...
        "reason": reason if (accepted and keep) else f"rejected: oracle={ora['oracle']} identity={ora['failure_identity']}",
        "n_atoms": len(s),
...
def require_freeze() -> None:
    if not FREEZE.is_file():
        raise RuntimeError("FREEZE_MANIFEST.json missing; minimization forbidden")
